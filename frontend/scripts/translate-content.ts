...
const TRANSLATION_ENGINE = process.env.TRANSLATION_ENGINE || (API_KEY ? 'google' : 'libretranslate');
  ...
  if (TRANSLATION_ENGINE === 'libretranslate') {
    ...
    for (let i = 0; i < toFetch.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.error(`Failed LibreTranslate chunk for '${targetLang}': ${err.message}`);
        console.warn(`Falling back to individual item translation for chunk...`);
        ...
        for (const original of chunk) {
          try {
            ...
            if (singleRes.ok) {
              ...
              failedStrings.push(`[${targetLang}] ${original.substring(0, 30)}...`);
            }
          } catch {
            failedStrings.push(`[${targetLang}] ${original.substring(0, 30)}...`);
          }
    ...
    if (!API_KEY) {
      ...
      console.error(`Please provide GOOGLE_TRANSLATE_API_KEY in .env OR run LibreTranslate on http://localhost:5000\n`);
      process.exit(1);
    }
    ...
    for (let i = 0; i < toFetch.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.error(`Failed Google Cloud chunk for target '${targetLang}':`, err.message);
...
async function run() {
  ...
  const { GUJARAT_DESTINATIONS, OFFICIAL_CATEGORIES } = await import('../src/data/destinations.js');
  const { UI_TRANSLATIONS } = await import('../src/data/translations.js');

  console.log(`Loaded ${GUJARAT_DESTINATIONS.length} destinations and ${Object.keys(UI_TRANSLATIONS).length} UI translation keys.`);
  ...
  console.log(`Found ${allTextsArray.length} unique English strings to translate.`);
  ...
  const updatedDestinations = GUJARAT_DESTINATIONS.map((dest: any) => {
    return {
      ...
      attractions: (dest.attractions || []).map((attr: any) => ({
        ...
        gujaratiDescription: attr.description ? (guMap.get(attr.description) || attr.description) : undefined,
        hindiDescription: attr.description ? (hiMap.get(attr.description) || attr.description) : undefined,
      })),
      hotels: (dest.hotels || []).map((hotel: any) => ({
        ...hotel,
        gujaratiName: guMap.get(hotel.name) || hotel.name,
        hindiName: hiMap.get(hotel.name) || hotel.name,
        gujaratiDescription: hotel.description ? (guMap.get(hotel.description) || hotel.description) : undefined,
        hindiDescription: hotel.description ? (hiMap.get(hotel.description) || hotel.description) : undefined,
      })),
  ...
  const updatedTranslations: Record<string, { en: string; gu: string; hi: string }> = {};
  for (const key of Object.keys(UI_TRANSLATIONS)) {
...
`;
  ...
  console.log(`Total Characters Sent to API: ${totalCharacters} / 500,000 free-tier monthly limit`);
