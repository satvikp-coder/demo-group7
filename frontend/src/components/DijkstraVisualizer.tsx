...
import { Play, Pause, SkipForward, RotateCcw, Cpu, CheckCircle, Info } from 'lucide-react';
...
export function getIntraCityGraph(cityId?: string): { nodes: GraphNode[]; edges: GraphEdge[] } {
  ...
  const attr4 = city?.attractions[3]?.name || `${cityName} Local Handicraft Market`;
  ...
  const nodes: GraphNode[] = [
    ...
    { id: 'attr2', name: attr2, shortName: attr2.split(' ')[0], x: 220, y: 270 },
    { id: 'attr3', name: attr3, shortName: attr3.split(' ')[0], x: 350, y: 110 },
    { id: 'attr4', name: attr4, shortName: attr4.split(' ')[0], x: 350, y: 250 },
...
export const DIJKSTRA_NODES: GraphNode[] = [
  { id: 'modhera', name: 'Modhera Sun Temple', shortName: 'Modhera', x: 220, y: 70 },
  { id: 'adalaj', name: 'Adalaj Ni Vav', shortName: 'Adalaj', x: 265, y: 125 },
  { id: 'ahmedabad', name: 'Sabarmati Heritage', shortName: 'Ahmedabad', x: 275, y: 175 },
  { id: 'champaner', name: 'Champaner-Pavagadh', shortName: 'Champaner', x: 375, y: 205 },
  { id: 'statue-of-unity', name: 'Statue of Unity', shortName: 'Statue of Unity', x: 415, y: 255 },
  { id: 'rann-of-kutch', name: 'Rann of Kutch', shortName: 'Rann of Kutch', x: 75, y: 75 },
  { id: 'dwarka', name: 'Dwarkadhish Temple', shortName: 'Dwarka', x: 65, y: 195 },
  { id: 'somnath', name: 'Somnath Shore Temple', shortName: 'Somnath', x: 135, y: 285 },
  { id: 'gir', name: 'Gir National Park', shortName: 'Gir', x: 200, y: 275 },
  { id: 'saputara', name: 'Saputara Hill Station', shortName: 'Saputara', x: 440, y: 315 },
];
...
export function generateDijkstraSteps(
  ...
  const effectiveStart = nodesList.some(n => n.id === startId) ? startId : (nodesList[0]?.id || 'hotel');
  const effectiveTarget = nodesList.some(n => n.id === targetId) ? targetId : (nodesList[nodesList.length - 1]?.id || 'resto');
  ...
  const getNeighbors = (nodeId: string) => {
    ...
    edgesList.forEach((edge) => {
      if (edge.from === nodeId) neighbors.push({ neighborId: edge.to, weight: edge.distanceKm });
      else if (edge.to === nodeId) neighbors.push({ neighborId: edge.from, weight: edge.distanceKm });
    });
    return neighbors;
  };
  ...
  const finalDist = distances[effectiveTarget] !== Infinity ? distances[effectiveTarget] : 0;
...
export const DijkstraVisualizer: React.FC<DijkstraVisualizerProps> = ({
  ...
  const [startId, setStartId] = useState<string>(startNodeId || activeNodes[0]?.id || 'hotel');
  const [targetId, setTargetId] = useState<string>(targetNodeId || activeNodes[activeNodes.length - 1]?.id || 'resto');
  ...
  useEffect(() => {
    const generated = generateDijkstraSteps(startId, targetId, activeNodes, activeEdges);
    setSteps(generated);
    ...
    <div className="bg-white border-2 border-gold p-4 sm:p-5 space-y-4 shadow-sm relative font-mono text-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone/30 pb-3">
        ...
        <div className="flex items-center gap-2 text-[11px]">
          ...
          >
            {activeNodes.map(n => (
              <option key={n.id} value={n.id}>Start: {n.shortName}</option>
            ))}
          </select>
          ...
          >
            {activeNodes.map(n => (
              <option key={n.id} value={n.id}>Target: {n.shortName}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      {hasReducedMotion && (
        <div className="p-2.5 bg-ink text-salt border border-stone/40 text-[11px] flex items-center gap-2">
          ...
          <span>Animation reduced per your system settings. Displaying computed final shortest path directly.</span>
        </div>
      )}
      ...
      <div className="bg-salt/90 border-2 border-stone/30 relative h-[320px] sm:h-[350px] w-full overflow-hidden p-2 shadow-inner">
        ...
        <svg className="w-full h-full" viewBox="0 0 500 360" preserveAspectRatio="xMidYMid meet" aria-label="Interactive Dijkstra Graph Visualization">
          ...
          {activeEdges.map((edge) => {
            ...
            const isInFinalPath =
              ...
              currentStep.finalPath.some((id, idx) => {
                ...
                return (id === edge.from && nextId === edge.to) || (id === edge.to && nextId === edge.from);
              });
            ...
            const isActive =
              ...
              ((currentStep.activeEdge.from === edge.from && currentStep.activeEdge.to === edge.to) ||
               (currentStep.activeEdge.from === edge.to && currentStep.activeEdge.to === edge.from));

            const isRelaxed =
              currentStep.relaxedEdge &&
              ((currentStep.relaxedEdge.from === edge.from && currentStep.relaxedEdge.to === edge.to) ||
               (currentStep.relaxedEdge.from === edge.to && currentStep.relaxedEdge.to === edge.from));
          ...
          {activeNodes.map((node) => {
            ...
            const isInFinalPath = currentStep.isComplete && currentStep.finalPath.includes(node.id);
      ...
      <div aria-live="polite" className="p-3 bg-ink text-salt border border-gold text-xs flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 truncate">
          <span className="bg-gold text-ink font-bold px-2 py-0.5 text-[10px] uppercase shrink-0">
            Step {currentStepIndex + 1}/{steps.length}
          </span>
          <span className="truncate text-stone text-[11px]">{currentStep.description}</span>
        </div>
        {currentStep.isComplete && (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-stone/30">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-label={isPlaying ? 'Pause algorithm execution' : 'Play algorithm execution'}
        ...
        <div className="flex flex-wrap items-center gap-3 text-[10px] text-stone">
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-full bg-gold inline-block border border-ink" /> Start/Current
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-full bg-stone/50 inline-block border border-gold" /> Frontier
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-full bg-ink inline-block border border-gold" /> Visited
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-full bg-madder inline-block border border-gold" /> Path
          </span>
        </div>
      </div>
