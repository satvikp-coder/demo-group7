...
export const AccessibilityBadge: React.FC<AccessibilityBadgeProps> = ({
  ...
  const filledSteps = physicalDemand === 'low' ? 1 : physicalDemand === 'moderate' ? 2 : 3;
    ...
    <div className="flex flex-wrap items-center gap-1.5 font-mono text-[10px] uppercase font-bold">
      {wheelchairAccessible !== undefined && (
        <span
          ...
          title={wheelchairAccessible ? 'Wheelchair accessible' : 'Not wheelchair accessible'}
        >
          <Accessibility
            className={`w-3.5 h-3.5 ${
              wheelchairAccessible ? 'text-emerald-700 font-bold' : 'text-stone/60'
            }`}
          />
          {!compact && (
            <span>{wheelchairAccessible ? 'Wheelchair Accessible' : 'Not Wheelchair Accessible'}</span>
          )}
        </span>
      )}

      {physicalDemand && (
        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 border ${demandStyles}`}>
          <div
