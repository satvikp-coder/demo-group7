...
export function checkBestTimeConflict(
  ...
  if (noteLower.includes('avoid midday') || noteLower.includes('open salt flat heat')) {
  ...
  if (noteLower.includes('10:15am') && (arrivalMins > 690 || arrivalMins < 570)) {
    return {
      hasConflict: true,
  ...
  if (noteLower.includes('4pm-6pm') && (arrivalMins < 930 || arrivalMins > 1110)) {
    return {
      hasConflict: true,
